...
import projectDWT.core.TimerMs;
...
public class WaveletSequentialApp {
    ...
    private final WaveletPipeline pipeline = new WaveletPipeline();

    public WaveletSequentialApp(){
        initUI();
    }

    private void initUI() {
        frame = new JFrame("Wavelet DWT Compression");
        ...
        Logger.info("App started. Available processors: " + Runtime.getRuntime().availableProcessors());
    ...
    private void runSequential(){
        ...
        Logger.info("Starting compression... threshold =" + thresholdPercent + "%");

        TimerMs timer = new TimerMs();
        timer.start();

        reconstructedImage = pipeline.compressAndReconstruct(originalImage, thresholdPercent);

        long elapsed = timer.stop();
        Logger.info("Sequential compression+decompression time: " + elapsed + " ms");
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(WaveletSequentialApp::new);
    }
}
